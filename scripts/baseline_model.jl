...
sweep_combos = vcat(
    ...
    [(train_length=tl, noise=0.0) for tl in TRAIN_LENGTHS if tl != 365],
)

for (train_length, noise) in sweep_combos
    ...
    for beta_function = [beta_exp, beta_mixed, beta_rational]
        ...
        if isfile(joinpath(data_dir, "results.jld2"))
            println("Simulation already exists for $(location) with noise=$(noise) and beta=$(beta_function). Skipping...")
            continue
        end
        mkpath(data_dir)
        #========================================================
