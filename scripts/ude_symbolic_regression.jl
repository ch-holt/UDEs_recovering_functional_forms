...
function symbolic_regression(I_nn, best_results, output_dir, input_size, location, seed)
    ...
    mach, r, best_equation, simplified_equation = run_symbolic_regression(SR_input_0_1, nn_output_0_1, output_dir; seed=seed)
...
for train_length in [365]
    for location in ["AK", "AL", "AR", "AZ", "CA", "CO", "CT", "DC", "DE" , "FL", "GA", "HI", "IA", "ID", "IL", 
        "IN", "KS", "KY", "LA", "MA", "MD", "ME", "MI", "MN", "MO", "MS", "MT", "NC", "ND", "NE", "NH", "NJ", "NM", "NV", "NY", "OH", "OK", "OR", "PA", "RI", "SD", "TN", "UT", "VA", "VT", "WI", "WV", "WY"]
        for noise in [0.0]
            ...
            data = JLD2.load(DrWatson.datadir("exp_pro","synthetic_data","synthetic_trajectories_$(beta_function)", "synthetic_$(location)", "noise=$(noise).jld2"))
            ...
            for folder in seed_folders
                ...
                plot_title = "SR on $(location), train_length=$(train_length), noise=$(noise)"

                # Find UDE results for this seed
                I_nn, results = extract_ude(root, obs, folder)

                # Use the same seed as the UDE initialisation (encoded in the folder name)
                seed = parse(Int, split(folder, "=")[2])

                t_start_sr = time()
                mach, r, best_equation, simplified_equation, SR_input_1000, SR_input_0_1, SR_input_days, nn_input_days, nn_output_days, nn_input_1000, nn_output_1000, nn_input_0_1, nn_output_0_1, I_grid, range_0_1, I_nn, norm_i_traj = symbolic_regression(I_nn, results, output_dir, input_size, location, seed)
